...
static unsigned long last_time_roll = 0;
...
float calcPID_Roll(float targetRoll, float currentRoll,float throttle) {
    ...
    float dt = (float)(now - last_time_roll) / 1000000.0f;
    ...
    if(throttle<1050.0f) roll_int=0.0f;
    I_error = roll_int;
    I_out = R_I * I_error;
...
float calcPID_Pitch(float targetPitch, float currentPitch,float throttle) {
    ...
    if (pitch_int < -50.0f) pitch_int = -50.0f;
    if(throttle<1050.0f) pitch_int=0.0f;
...
float calcPID_Yaw(float targetYaw, float currentYaw,float throttle) {
    ...
    if (yaw_int < -50.0f) yaw_int = -50.0f;
    if(throttle<1050.0f) yaw_int=0.0f;
